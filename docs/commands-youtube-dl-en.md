...
Create a new playlist in a database which you can add songs and play them later. Every playlist is _public_ by default.
...
Sends a message to see all the songs from a playlist. The access is denied if is private (unless is _your_ playlist). You can navigate through pages using emojis.
